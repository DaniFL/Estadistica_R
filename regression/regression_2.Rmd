...
# usar ANOVA tipo III
library("car")  
source("regression/utils.R")

data("iris")

# 1) Visualizar da la vuelta a las coordenadas del otro ejercicio
head(iris)
...
# Esto construye contrastes ortogonales
...
summary(v_model)
confint(v_model)
# No tiene sentido tiene mas sentido al haber hecho anova hacer test post hoc que compara todo con todo
```
...
# H0: rojo Vs (azul y verde)
# H0: mu_setosa = .....
# H0: 0.5 * mu_versicolor + 0.5 * mu_virginca - mu_setosa != 0

# H0 2: mu_versicolor - mu_virginica = 0

# Hacemos una listas de contrastes
contrast_list = list('V-Setosa' = c(-1, 0.5, 0.5), 'I - II' = c(0, 1, -1))

# contrasts    contrast -> contraste usando un modelo de emmeans
...
# 1) Vis...
library(readr)
anxiety = read_csv("regression/data/anxiety.csv")
anxiety$group = factor(anxiety$group)

anxiety$diferencia = anxiety$posttest - anxiety$pretest
...
ggplot(anxiety, aes(x = group, y = diferencia, fill = group)) + geom_boxplot()
# 2) Contrates 
# No tenemos hipótesis :(
contrasts(anxiety$group) = contr.helmert(3)
contrasts(anxiety$group)

# contrastes ortogonales si o si
...
anxiety_means = emmeans(anxiety_lm, "group")
anxiety_anova = Anova(anxiety_lm, type = 3)

posttest = -0.94 + 1.02775 * pretest + (-0.64) * group2 + (-2.98)* group3

# Grupo 1: 
posttest = -0.94 + 1.02775 * pretest
# Grupo 3:
posttest = -0.94 + 1.02775 * pretest + -2.98 = (-0.94 - 2.98) + 1.02 * pretest

# 4) Posthoc tests!
pairs(anxiety_means, adjust = "bonferroni", infer = c(TRUE, TRUE))
```
...
```{r}
drugs_df_1 = read.csv("regression/data/drugs_1.csv")
...
```{r}
# Como influye el sexo y la droga en el tiempo de reacción
...
# Modelo sin interacción == Paralelismo
# Como no hemos hecho contrastes utiliza parámtros por defecto, (Mujer y DrogaA)
# P_valores bajos
# los hombres están por debajo de las mujeres en todas las drogas 
```
...
drugs_df_2 = read.csv("regression/data/drugs_2.csv")
...
```{r}
# Se multiplica para que exista una interacción entre las variables
drugs_model_2 = lm(response_time ~ sex * drug, data = drugs_df_2) 
...
# Se añaden dos filas más a la tabla de coeficientes
# sexMale:drugB       -20.2263     0.7498  -26.98  < 2e-16 ***
# sexMale:drugPlacebo -19.5854     0.7498  -26.12  < 2e-16 ***

# response = 23 + 15 * sexMale + 11 * drugB -9 * placebo + -20 * sexMale:drugB + -19 * sexMale:drugPlacebo
...
# si las interacciones son significativas, no tiene sentido mirar los efectos principales (coeficientes principales)
```
...
# Anova necesita contrastes ortogonales (forma1 - matriz)
contrasts(drugs_df_2$drug)
# Drugs Vs Placebo: H0: 0.5 * (mu_A + mu_B) - mu_placebo = 0.    (OJO CON EL ORDEN)
# A Vs B: H0: mu_A - mu_B = 0
...
# 3) ANOVA 
drugs_model_2 = lm(response_time ~ drug * sex, data = drugs_df_2)
...
summary(drugs_model_2)
# funcion constrast necesita una lista de contrastes, mientras que constrasts necesita una matriz
drug_means = emmeans(drugs_model_2, ~ drug) 
contrast(drug_means, method = list("drugs" = drugs_contrasts_list))

# medias para interacciones 
conditional_means = emmeans(drugs_model_2, ~ drug | sex) # Medias condicionadas al sexo
contrast(conditional_means, interaction = list("drugs" = drugs_contrasts_list, "sex" = "consec"), # "consec" = consecutivos
         by = NULL)
```
...
drugs_emms = emmeans(drugs_model_2, ~ drug | sex) # media de cada droga condicionada al sexo
contrast(drugs_emms, interaction = list("drugs" = "pairwise", "sex" = "consec"),  by = NULL, adjust = "fdr")
# Conclusiones
# La droga A es más efectiva que la B y el placebo
```
